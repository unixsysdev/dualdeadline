...
\begin{itemize}
  ...
  low-rank materialization, \texttt{torch.compile}, and a custom Triton GEMV,
  with negative results reported.
\end{itemize}
...
deadline, rather than capacity, is binding.
...
simpler loss.  Three training seeds yield 44.71\% mean R@4 with 0.04-point
...
error is below $3\times10^{-7}$.  On the direct-embedding layer-aware model,
\texttt{torch.compile} is slower in an isolated run (0.137\,ms versus
0.078\,ms eager).  These results argue for measuring small-kernel launch
behavior rather than assuming a compiler or custom kernel must win
