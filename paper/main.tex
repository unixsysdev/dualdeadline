...
$1.223\ms$ exposed stall for monolithic prefetch and $1.100\ms$ for
...
\begin{itemize}
  ...
  low-rank materialization, CUDA Graphs, \texttt{torch.compile}, and a custom
  Triton GEMV, with negative results reported.
\end{itemize}
...
0.81--0.89\,ms before predictor cost; the primary predictor costs 0.092\,ms.
Thus four gate/up transfers are feasible on 14 of 15 target layers and five
on one layer.
...
At layer 0, varying routed top-$k$ from 1, 2, 4, to 8 gives absolute staging
savings of 0.063, 0.061, 0.122, and 0.254\,ms, respectively.
...
1.223\,ms per layer-token and staging exposes 1.100\,ms; the paired difference
is -0.123\,ms (95\% CI [-0.125,-0.122]).  The disjoint replication gives
1.207\,ms and 1.081\,ms, a -0.126\,ms difference
([-0.1265,-0.1252]).  Larger byte budgets do not help because the measured
deadline, rather than capacity, is binding.  Replacing the mean-copy fit with
a p95-copy fit preserves a -0.112\,ms staging advantage.
...
monolithic and 0.428\,ms staged; at capacity 64 it is 0.055\,ms and
0.040\,ms.  Intermediate capacities 16 and 32 give
0.335/0.242\,ms and 0.139/0.092\,ms, respectively.
...
training seeds, pilot R@4 is 45.69\% with a 0.40-point sample standard
deviation; the corresponding disjoint-replication result is 46.92\% with a
0.25-point standard deviation.
...
error is below $3\times10^{-7}$.  On the primary conformant layer-aware model,
\texttt{torch.compile} is slower in an isolated run (0.140\,ms versus
0.092\,ms eager).  In contrast, a static CUDA Graph costs 0.0220\,ms to replay
and 0.0325\,ms including device-side input copies, versus 0.0938\,ms eager,
with identical output.  The primary deadline analysis conservatively retains
the measured eager cost.  These results argue for measuring small-kernel
launch behavior rather than assuming a compiler or custom kernel must win
\cite{tillet2019triton}.

\section{Limitations}
Our end-to-end claims are deliberately narrow.  Trace replay assumes one DMA
channel and uses measured p50 windows; only the expert-block overlap is
executed with real streams.  We do not yet integrate staged components into a
production scheduler, model PCIe contention from other requests, or measure
batched serving throughput.  The cache simulator gives speculative weights a
separate workspace and resets LRU state per prompt.  OLMoE is smaller than
current frontier MoEs, and results must not be extrapolated to trillion-
parameter models without measurement.  The disjoint replication, low-rank
control, and several systems ablations were added after the preregistered pilot
and are labeled exploratory in the machine-readable protocol.
